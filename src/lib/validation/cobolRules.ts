...
export const DivisionAndSectionRule: CobolRule = {
  evaluateLine(line, lineNum, context) {
    ...
    const areaA =
      line.length >= 8 ? line.substring(7, Math.min(line.length, 11)) : ''
    const codeArea =
      line.length >= 8 ? line.substring(7, Math.min(line.length, 72)) : ''
    ...
    for (const div of COBOL_SPECS.ALL_DIVISIONS) {
      if (uppercaseCode.includes(div)) {
        if (div === 'IDENTIFICATION DIVISION')
          context.hasIdentificationDivision = true
        if (areaA.trim().length === 0) {
          diags.push({
            line: lineNum,
            column: 8,
            message: `RC 08 (E) - Division header '${div}' must start in Area A (Columns 8-11).`,
            severity: 'error',
          })
        }
        if (!trimmedCode.endsWith('.')) {
          diags.push({
            line: lineNum,
            column: line.indexOf(div) + div.length + 1,
            message: `RC 08 (E) - Division header '${div}' must end with a period '.'.`,
            severity: 'error',
          })
        }
      }
      ...
      if (uppercaseCode.includes(sec)) {
        if (areaA.trim().length === 0) {
          diags.push({
            line: lineNum,
            column: 8,
            message: `RC 08 (E) - Section header '${sec}' must start in Area A (Columns 8-11).`,
            severity: 'error',
          })
        }
        if (!trimmedCode.endsWith('.')) {
          diags.push({
            line: lineNum,
            column: line.indexOf(sec) + sec.length + 1,
            message: `RC 08 (E) - Section header '${sec}' must end with a period '.'.`,
            severity: 'error',
          })
        }
      }
    ...
    if (uppercaseCode.includes('PROGRAM-ID')) {
      ...
      if (areaA.trim().length === 0) {
        diags.push({
          line: lineNum,
          column: 8,
          message: `RC 08 (E) - PROGRAM-ID paragraph header must start in Area A (Columns 8-11).`,
          severity: 'error',
        })
      }
      if (!trimmedCode.endsWith('.')) {
        diags.push({
          line: lineNum,
          column: line.indexOf('PROGRAM-ID') + 11,
          message: `RC 08 (E) - PROGRAM-ID paragraph header must end with a period '.'.`,
          severity: 'error',
        })
      }
    }
      ...
      if (isAreaALevel) {
        if (firstWordCol >= 12) {
          diags.push({
            line: lineNum,
            column: firstWordCol,
            message: `RC 04 (W) - Top-level data indicator '${levelNum}' should start in Area A (Columns 8-11).`,
            severity: 'warning',
          })
        }
      } else {
        if (firstWordCol < 12) {
          diags.push({
            line: lineNum,
            column: firstWordCol,
            message: `RC 08 (E) - Sub-level data item '${levelNum}' must start in Area B (Column 12 or later).`,
            severity: 'error',
          })
        }
      }

      const hasClauses = COBOL_SPECS.DATA_CLAUSES.some((c) =>
        uppercaseCode.includes(c),
      )
      if (hasClauses) {
        const namePart = trimmedCode.substring(
          0,
          trimmedCode.search(
            new RegExp(`\\b(${COBOL_SPECS.DATA_CLAUSES.join('|')})\\b`, 'i'),
          ),
        )
        if (namePart.includes('.')) {
          diags.push({
            line: lineNum,
            column: line.indexOf('.'),
            message: `RC 08 (E) - Premature period inside data item definition before clauses. The entire variable definition must end with a single period at the end.`,
            severity: 'error',
          })
        }
        if (!trimmedCode.endsWith('.')) {
          diags.push({
            line: lineNum,
            column: line.length,
            message: `RC 08 (E) - Data item definition '${varName}' must end with a period '.'.`,
            severity: 'error',
          })
        }
      }
...
export const UndefinedVariableRule: CobolRule = {
  evaluateLine(line, lineNum, context) {
    ...
    if (
      firstCodeWord &&
      COBOL_SPECS.ARITHMETIC_STATEMENTS.includes(firstCodeWord)
    ) {
      const cleanedCode = trimmedCode.replace(/['"][^'"]*['"]/g, '')
      const words = Array.from(cleanedCode.matchAll(/[A-Za-z0-9-]+/g)).map(
        (m) => m[0],
      )
      ...
      for (const word of words) {
        ...
        if (
          uWord &&
          !/^\d+$/.test(uWord) &&
          !COBOL_SPECS.RESERVED_WORDS.has(uWord)
        ) {
          if (!context.declaredVariables.has(uWord)) {
            diags.push({
              line: lineNum,
              column: line.indexOf(word) + 1,
              message: `RC 12 (S) - Undefined identifier '${word}'. Variable used without being declared in WORKING-STORAGE.`,
              severity: 'error',
            })
          }
        }
...
export const StopRunRule: CobolRule = {
  evaluateLine(line, lineNum) {
    ...
    if (uppercaseCode.includes('STOP RUN') && !trimmedCode.endsWith('.')) {
      diags.push({
        line: lineNum,
        column: line.toUpperCase().indexOf('STOP RUN') + 9,
        message: `RC 08 (E) - STOP RUN statement must end with a period '.'.`,
        severity: 'error',
      })
    }
    return diags
    ...
    for (const word of words) {
      if (word.length > 1 && (word.startsWith('-') || word.endsWith('-'))) {
        diags.push({
          line: lineNum,
          column: line.indexOf(word) + 1,
          message: `RC 08 (E) - User-defined word '${word}' cannot begin or end with a hyphen.`,
          severity: 'error',
        })
      }
    }
...
export const StringLiteralRule: CobolRule = {
  evaluateLine(line, lineNum) {
    ...
    const indicatorChar =
      line.length >= 7 ? line[COBOL_SPECS.COLUMNS.INDICATOR] : ' '
    if ((inSingleQuote || inDoubleQuote) && indicatorChar !== '-') {
      diags.push({
        line: lineNum,
        column: 8,
        message: `RC 12 (S) - Unclosed string quote in COBOL literal. Continuation lines require '-' in Column 7.`,
        severity: 'error',
      })
    }
    return diags
...
export const LineLengthRule: CobolRule = {
  evaluateLine(line, lineNum) {
    const diags: Diagnostic[] = []
    if (
      line.length > COBOL_SPECS.COLUMNS.MAX_LINE_LENGTH &&
      line.substring(COBOL_SPECS.COLUMNS.MAX_LINE_LENGTH).trim().length > 0
    ) {
      diags.push({
        line: lineNum,
        column: 73,
        message: `RC 04 (W) - Code beyond Column 72 (Identification Area) is ignored by standard COBOL compilers.`,
        severity: 'warning',
      })
    }
    return diags
  ...
  evaluateGlobal(context) {
    ...
    if (!context.hasIdentificationDivision) {
      diags.push({
        line: 1,
        column: 1,
        message: `RC 12 (S) - Missing required IDENTIFICATION DIVISION header.`,
        severity: 'error',
      })
      ...
      diags.push({
        line: 1,
        column: 1,
        message: `RC 12 (S) - Missing required PROGRAM-ID paragraph in IDENTIFICATION DIVISION.`,
        severity: 'error',
      })
    }
    return diags
